package com.fujitsu.bss.app.powershell.remote.app;
...
public class AesEncrypterWithKeyFile {

    private static final Logger LOG = LoggerFactory.getLogger(AesEncrypterWithKeyFile.class);
	...
	AesEncrypterWithKeyFile.key = new SecretKeySpec(key, "AES");
    }
	...
	if (keyFile.exists()) {
		...
		AesEncrypterWithKeyFile.setKey(Arrays.copyOfRange(key, 0, AesEncrypterWithKeyFile.KEY_BYTES));
	    } catch (IOException | ArrayIndexOutOfBoundsException e) {
		throw new GeneralSecurityException("Keyfile at " + keyFile.getAbsolutePath() + " is not readable");
	    }
	} else {
	    throw new GeneralSecurityException("Keyfile not accessible. " + keyFile.getAbsolutePath());
	}
    }
     ...
     *
     * @param encrypted
     *            the encrypted text. optionally leaded by the iv and separated with
     *            ':'.
     * @return the string
     */
