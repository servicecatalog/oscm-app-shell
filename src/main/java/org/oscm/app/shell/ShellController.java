...
public class ShellController implements APPlatformController {
        ...
        private void validateScript(Configuration config,
                ...
                if (scriptFilename.isEmpty()) {
                        throw new APPlatformException(
                                "Failed to read service parameter "
                                        + scriptKey);
                }
                ...
                if (text.indexOf("END_OF_SCRIPT") < 0) {
                        throw new APPlatformException(
                                "Missing output \"END_OF_SCRIPT\" in "
                                        + scriptFilename);
                }
        }
        ...
        void runVerificationScript(Configuration config)
                ...
                try {
                        script = new Script(
                                config.getSetting(VERIFICATION_SCRIPT));
                        script.insertServiceParameters(
                                config.getProvisioningSettings());
                        scriptLogger
                                .logScriptConfiguration(config, "VERIFICATION",
                                        script.get());
                } catch (Exception e) {
                        return;
                }
